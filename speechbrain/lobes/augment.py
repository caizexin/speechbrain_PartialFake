...
# Author: Weiqing Wang
class DiacAugment(torch.nn.Module):
    """An implementation of Diaconis augmentation algorithm.

    Reference:
        https://arxiv.org/abs/1910.09703

    Arguments:
        rotation_probability: float
            The probability of rotation
        eps: float
            check if the embd is l2 normalized by norm(embd) - torch.ones() < eps

    """
    def __init__(self, eps=1e-6):
        super().__init__()
        from scipy.stats import ortho_group # noqa

        self.ortho_group = ortho_group
        self.eps = eps # not finished

    def get_rotation(self, dim):
        """ get a random orthonormal matrix for rotation with rank=dim
        dim: int
            the size of the rotation matrix
        return: tensor [dim x dim]
            An orthonormal matrix with size of dim x dim
        """
        return self.ortho_group.rvs(dim=dim)

    def rotate(self, x, T):
        """ rotate the input x with orthonormal basis T
        x: torch.Tensor [... x dim]
            l2-normalized embeddings with last dimension = dim
        T: torch.Tensor [dim x dim]
            An orthonormal matrix with size of dim x dim
        return: torch.Tensor [... x dim]
            the embeddings after rotation
        """
        return torch.matmul(x, T)

    def check_l2(self, x):
        """ check if the input x is l2-normalized in the last dimension
        x: torch.Tensor [... x dim]
        return: bool
        """
        if torch.any(torch.abs(torch.norm(x, dim=-1, p=2)-1) > self.eps):
            return False

        return True


    def forward(self, embds, T=None):
        """Returns the rotated embeddings.

        Arguments
        ---------
        embd: torch.Tensor [... x dim]
            The l2-normalized embeddings to rotate
        """
        if not self.check_l2(embds):
            raise ValueError("The embeddings are not l2-normalized.")

        embd_dim = embds.shape[-1]
        if T is None:
            T = torch.from_numpy(self.get_rotation(embd_dim)).float()
        T = T.to(embds.device)

        return self.rotate(embds, T), T
